...
export const Login: React.FC<LoginProps> = ({ onLogin, onSwitchToSignup }) => {
  ...
  const handleGoogleLogin = () => {
    ...
    // Simulate OAuth network delay and successful return
    setTimeout(() => {
        const googleUser: UserProfile = {
            email: 'google.student@university.edu',
            nickname: 'Google Student',
            avatarId: 1, // Different avatar for differentiation
            uniqueId: '#GGL24',
            location: null,
            stats: {
                focusMinutes: 320,
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-3 pt-2">
                    ...
                    <button 
                        ...
                        className={`w-full py-4 rounded-xl font-bold text-stone-700 bg-white border-2 border-stone-100 hover:bg-stone-50 transition-colors flex items-center justify-center space-x-2 ${isGoogleLoading ? 'opacity-75 cursor-wait' : ''}`}
                    >
                        {isGoogleLoading ? (
                            <>
